  ...
  switch (currentMode) {
    ...
    case MODE_BLINK:
      if (millis() - lastBlinkMillis >= BLINK_INTERVAL) {
        ...
        if (yelFixedOn) digitalWrite(LED_YELLOW, HIGH);
        else if (yelFixedOff) digitalWrite(LED_YELLOW, LOW);
        else digitalWrite(LED_YELLOW, blinkOn ? HIGH : LOW);
        if (grnFixedOn) digitalWrite(LED_GREEN, HIGH);
        else if (grnFixedOff) digitalWrite(LED_GREEN, LOW);
        else digitalWrite(LED_GREEN, blinkOn ? HIGH : LOW);
      }
      break;
  }
}
